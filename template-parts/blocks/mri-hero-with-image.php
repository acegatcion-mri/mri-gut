<?php
/**
 * MRI Hero With Image block template.
 *
...
$shared_fields = pardot_boostrap_get_shared_block_fields( 'mri_hero_with_image', $block );
$heading       = $shared_fields['heading'];
$content       = $shared_fields['content'];
$logo_image = pardot_boostrap_get_block_field_value( $block, 'mri_hero_with_image_logo_image' );
$hero_image = $shared_fields['image'];
$benefits = pardot_boostrap_get_block_field_value( $block, 'mri_hero_with_image_benefits' );
...
$block_id = pardot_boostrap_setup_block_id_and_preview( $block, $is_preview, 'mri-hero-with-image' );
?>


<section id="<?php echo esc_attr( $block_id ); ?>" class="hero position-relative mri-hero-with-image">
    ...
    <div class="row align-items-center g-5">
      ...
      <div class="col-lg-6">
        <?php if ( ! empty( $hero_image['url'] ) ) : ?>
          <img src="<?php echo esc_url( $hero_image['url'] ); ?>" alt="<?php echo esc_attr( $hero_image['alt'] ); ?>" class="img-fluid" />
        <?php endif; ?>
      </div>
    </div>
  </div>
